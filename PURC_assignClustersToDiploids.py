...
## then annotates all samples in the matrix with the nearest diploid. It also uses linear_sum_assignment from scipy to minimize
## the total distance between all sequences of each sample in a pairwise fashion, and exports results as a distance matrix.
## Also creates output presence/absence file that can be used to create an UpSet plot in R (package 'UpSetR').
...
##		Outputs "Summary" files: one for each unique taxon or taxa combination, and a distance matrix at the sample level (minimized pairwise distance between all sequences in all samples)
...
# Loop through distance dictionary to extract branch lengths for sets of sequences that belong to a pair of samples. Write minimum OTU distance to all-by-all sample matrix
		...
		for headerName in sorted(set(polyploidCombosDict.keys())):
			polyploidDistOutfile.write(",%s" %(headerName))
